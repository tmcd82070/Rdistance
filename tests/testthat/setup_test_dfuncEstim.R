...
test_dfuncEstim <- function( detectParams,
  ...
  for( i in 1:nrow(detectParams) ){
    testParams <- paste0("i=", i, "/", nrow(detectParams),
    ...
    if( detectParams$x.scl[i] == "max" ){
      ...
      param.x.scl = units::set_units(as.numeric(detectParams$x.scl[i]), "m")
    }
    param.w.lo  = units::set_units(as.numeric(detectParams$w.lo[i]), "m")
        ...
        test_that("x.scl less than w.lo warning", {
          expect_warning(
            dfuncEstim(formula = formula,
                       ...
                       w.lo = param.w.lo,
                       w.hi = units::set_units(detectParams$w.hi[i], "m"), 
                       pointSurvey = detectParams$pointSurvey[i], 
                       expansions = detectParams$expansions[i], 
                       x.scl = param.x.scl, 
    ...
    if( inherits(tmp, "warning") | inherits(tmp, "error") ){
      ...
      test_that("Under singular var-cov warning", {
        ...
        test_that("Strip", {
          lo <- units::set_units( units::set_units(detectParams$w.lo[i], "m"), detectParams$outputUnits[i], mode = "standard" )
          hi <- units::set_units( units::set_units(detectParams$w.hi[i], "m"), detectParams$outputUnits[i], mode = "standard" )
        ...
        test_that("Scaling", {
          if( !is.character(param.x.scl) ){
            x0 <- format(units::set_units( param.x.scl,  detectParams$outputUnits[i], mode = "standard" ))
          } else {
