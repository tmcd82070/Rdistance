...
testthat::test_that("Triangle w/ cont cov, same value", {
...
testthat::test_that("Triangle, no covar, wlo 20, whi 150",{
  fit <- sparrowDf |> dfuncEstim(formula = dist ~ 1 + groupsize(groupsize)
                                 , likelihood = lhood
                                 , w.lo = w.20
                                 , w.hi = w.hi
...
testthat::test_that("Triangle, no covar, wlo 20, whi high",{
  fit <- sparrowDf |> dfuncEstim(formula = dist ~ 1 + groupsize(groupsize)
                                 , likelihood = lhood
                                 , w.lo = w.20
                                 # , w.hi = w.hi
...
testthat::test_that("Triangle, no covar, ft",{
  fit <- sparrowDf |> dfuncEstim(formula = dist ~ 1 + groupsize(groupsize)
                                 , likelihood = lhood
                                 , outputUnits = "ft"
  )
