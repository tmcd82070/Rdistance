...
correct.params <- c(Sigma = 43.584314, k= 2.405927, 
                    22.12881, 1.18376000,
                    1.18376, 0.09590859)
correct.esw <- units::set_units(61.11514, "m")
correct.nHat <-  8.431263
tol <- 0.00001

testthat::test_that("Hazrate:params", {
  testthat::expect_equal(object = c(fit$parameters, fit$varcovar)
                         , expected = correct.params 
                         , tolerance = tol)
})

testthat::test_that("Hazrate:esw", {
  testthat::expect_equal(object = effectiveDistance(fit)
                         , expected = correct.esw
                         , tolerance = tol)
})

testthat::test_that("Hazrate:nHat", {
