...
Estimating abundance requires the additional information contained in the second required dataset, described earlier, where each row reprensents one transect. Load the example dataset of surveyed sparrow transects from the package.
```{r}
...
auto <- F.automated.CDA(detection.data=sparrow.detections, transect.data=sparrow.transects,
                        w.hi=150, plot=FALSE, area=10000, R=500, ci=0.95, plot.bs=TRUE)
```
