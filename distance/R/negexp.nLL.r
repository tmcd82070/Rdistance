negexp.nLL <- function(a, dist, w.lo=0, w.hi=max(dist), series, expansions=0){
#
#   Compute the negative log likelihood for negative exponential density.
#
#   Input:
#   a = parameter values. Length and meaning depend on series and expansions.
#       a must be at least length = 1.  a[1] is rate parameter for exponential distribution.
#       a[2:length(a)] are coefficients
#       of expansion terms.
...
#	
    ...
    LL = negexp.like( a, dist, w.lo=w.lo, w.hi=w.hi, series, expansions )
