...
of animals over all occasions at camera i, and $n$ is the total count of animals over all cameras and occasions. We then estimated the variance of $\hat{N}$ using the delta method. This assumes probability of detection of 1.0; otherwise, there is an additional component for estimating $\hat{P}$.  

### Fewster et al 

Fewster et al (2009) state the following: "Overall, taking account both of estimator performance and
simplicity, we recommend estimator R2 for random survey designs. For systematic designs, we recommend O2 when the
design can accommodate overlapping strata in whole or part, and S2 otherwise. Estimator R2 is used for the within-stratum
variances in S2 and O2 (equations (13) and (16))."

Estimator R2 for random placement of transects is their Equation (3), 
$$
\widehat{var}(\frac{n}{L}) = \frac{k}{L^2(k-1)}\sum_{i=1}^k l_i^2 \left( \frac{n_i}{l_i} - \frac{n}{L}\right)^2
$$
where $k$ is number of transects, $l_i$ is length of transect $i$, $n_i$ is number of targets on transect $i$, $n = \sum_i n_i$ and $L=\sum_i l_i$.

Fewster et al's (2009) estimator S2 for systematic transect placement is their equation 13, 
$$
\widehat{var}(\frac{n}{L}) = \frac{1}{L^2}\sum_{h=1}^H L_h^2 \widehat{var}_h(\frac{n_h}{L_h}).
$$
Here we assume the systematic placement of transects has been post-stratified into $H$ stratum, each with $k_h$ lines in them, $n_h$ encounters (sighted targets), and $L_h$ total line length. Within post-stratum variance estimate $\widehat{var}_h(\frac{n_h}{L_h})$ is their R3 estimator (above). 
...
*   You have read Fewster et al 2009. Dig into `Distance` and see whether these formula are implemented. Especially, see how `Distance` estimates the other component, $\widehat{var}(\frac{1}{\hat{P}})$, variance of detection probability. 
*   It would be interesting to compare these estimators to the bootstrap estimators used in `Rdistance`.
