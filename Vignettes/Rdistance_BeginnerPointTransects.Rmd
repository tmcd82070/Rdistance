...
Next, we fit a detection function (plotted as a red line) using `dfuncEstim`
with point transects specified as an option (i.e., `PointSurvey` = `TRUE`). Our illustration uses the half-normal likelihood as the detection function. In section 5, we demonstrate an automated process that fits multiple detection functions and compares them using AICc. 
