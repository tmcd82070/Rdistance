...
1.  Install `Rtools`: Download the correct `Rtools` installer for your
    version of R from
    [here](https://cran.r-project.org/bin/windows/Rtools/). When
    download completes, run the installer (double-click it) and accept
    all default options. If `Rtools` is already installed, skip this
    step.
...
[here](https://cran.r-project.org/bin/linux/ubuntu/fullREADME.html).
...
2.  `Rdistance` requires the `units` package, and the `units` package
    requires the `udunits` library. Install the `udunits` library on
    linux via:
...
Once R is installed and running, install the current stable release or
...
Our [NEWS](NEWS.md) file contains salient changes across version
numbers.
