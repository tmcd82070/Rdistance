...
g.neg <-  function(x, params, covars, like, w.lo=0, w.hi=max(dist), series, expansions=0){

    f.like <- match.fun(paste( like, ".like", sep=""))

    g.x <- f.like( params, x, covars, w.lo=w.lo, w.hi=w.hi, series, expansions )
...
x.max <- optim(x.start, g.neg,  params = fit$parameters, method = "L-BFGS-B", w.lo=fit$w.lo, w.hi=fit$w.hi, like=fit$like.form,
    expansions=fit$expansions, series=fit$series, lower=fit$w.lo, upper=fit$w.hi, covars = covars)
