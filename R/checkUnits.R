...
checkUnits <- function(ml){
    ...
    # Units for dist ----
    if ( !inherits(ml$distances$dist, "units") & ml$Rdist_requireUnits ){
        ...
        stop(paste("Measurement units required on distances.",
            ...
            paste0("units(",ml$dfName,"$", respName, ")"), "<- '<units of measurment>',\n",
            "for example 'm' (meters) or 'ft' (feet). See units::valid_udunits()"))
    } else if ( ml$Rdist_requireUnits ){
        ...
        if ( !is.null(ml$outputUnits) ){
            ...
            ml$outputUnits <- units(ml$distances$dist)
        }
    }
    
    # Units for w.lo ----
    if ( !inherits(ml$w.lo, "units") & ml$Rdist_requireUnits ){
    ...
    # Units on w.hi ----
    if (is.null(ml$w.hi)){
        ml$w.hi <- max(ml$distances$dist, na.rm = TRUE) # units flow through max() automatically
    } else if ( !inherits(ml$w.hi, "units") & ml$Rdist_requireUnits ){
