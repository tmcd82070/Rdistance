...
#' but can look similar to a uniform if the data warrant.  
#' The uniform likelihood is actually the 
...
#' 
#' Parameter \eqn{a} can be thought of as the location of the 
#' "approximate upper limit" of an uniform distribution's 
#' support by noting that the inverse likelihood of 0.5 
#' is \code{a} before scaling 
#' (i.e., \code{uniform.like(c(a,b),a,scale=FALSE)} equals 
#' \code{0.5}). 
#' 
#' Parameter \code{b} is the "knee" or sharpness 
...
#' is large, the "knee" is sharp and the density of observations
#' declines rapidly at \eqn{a/f(0)}. In this case, the likelihood 
#' can look like a uniform distribution with support from 
...
#' \deqn{f(x|a,b,c_1,c_2,\dots,c_k) = f(x|a,b)(1 + \sum_{j=1}^{k} c_j h_{ij}(x)).}{%
